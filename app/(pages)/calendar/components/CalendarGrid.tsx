            ...
            {days.map((date) => {
                ...
                return (
                    <div
                        key={dateStr}
                        onClick={() =>
                            isCurrentMonth && setExpandedDay(dateStr === expandedDay ? null : dateStr)
                        }
                        className={`min-h-[100px] p-2 text-xs transition-all group
            ${isCurrentMonth ? 'cursor-pointer bg-blue-300/5 hover:bg-blue-300/10' : 'bg-blue-300/5'}
            ${isToday && isCurrentMonth ? 'border-2 border-orange-400 shadow-md z-10 relative rounded-md' : ''}`}
                    >
                        {isCurrentMonth ? (
                            <>

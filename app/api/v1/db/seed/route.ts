/**
 * API Endpoint: /api/v1/db/seed
 *
 * Methods:
 *  - POST: Run database seed on specified environment
 ...
 *    {
 *      "success": true,
 *      "message": "Seed completed for dev",
 *      "environment": "dev"
 *    }
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Temporarily set DATABASE_URL for seeding
    ...
    try {
      ...
      return NextResponse.json({
        success: true,
        message: `Seed completed for ${environment}`,
        environment,
      });
    } catch (error) {
      return NextResponse.json(
